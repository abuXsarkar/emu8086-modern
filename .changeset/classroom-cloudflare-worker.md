- **Cloudflare Workers + Durable Objects deployment for the
  classroom relay**. New `@emu8086/classroom-server-worker` package
  ...
  EMU8086_CLASSROOM_HMAC_SECRET` → `wrangler deploy`. Free tier
  ...
  into `@emu8086/classroom-server/protocol-helpers` so both
