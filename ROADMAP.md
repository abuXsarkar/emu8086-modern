...
| [M2](#m2--assembler-emu8086-dialect) | Assembler (emu8086 dialect) | ✅ shipped | Lex+parse+encode for nearly every M1 mnemonic, `org`/`db`/`dw`/`equ`/`dup`/BYTE-WORD PTR, user `MACRO`/`ENDM`, file-level `include`, `.MODEL`/`.STACK`/`.DATA`/`.CODE`/`.STARTUP`/`.EXIT`/`ASSUME`/`END` directives, `PROC`/`ENDP` blocks. |
...
| [M4](#m4--time-travel-debugger--devices) | Time-travel + devices | ✅ shipped (alpha) | step_back via diff snapshots; 7-seg, traffic-light, 8×8 LED matrix, stepper motor, text-mode screen (B800:0000), keyboard FIFO (ports 0x60/0x64 + INT 16h/INT 21h) peripherals. |
